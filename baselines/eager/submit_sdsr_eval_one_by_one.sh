...
echo "Submitting ${#DATASETS[@]} normal eval jobs, one dataset per bsub"
for DATASET in "${DATASETS[@]}"; do
  export DATASET
  JOB_SAFE_DATASET="${DATASET//[^A-Za-z0-9_]/_}"
  echo "Submitting ${DATASET}"
  bsub \
    -J "eager_sdsr_eval_${JOB_SAFE_DATASET}" \
    -q gpu \
    -gpu "num=1:mode=exclusive_process" \
    -n "${CPU_CORES:-1}" \
    -R "rusage[mem=${MEM_MB:-32000}]" \
    -W "${WALLTIME:-12:00}" \
    -o "${PROJECT_ROOT}/lsf_logs/eager_eval_${JOB_SAFE_DATASET}_%J.out" \
    -e "${PROJECT_ROOT}/lsf_logs/eager_eval_${JOB_SAFE_DATASET}_%J.err" \
