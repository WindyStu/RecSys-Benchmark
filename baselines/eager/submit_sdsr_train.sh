...
MAX_CONCURRENT_GPU="${MAX_CONCURRENT_GPU:-5}"
...
echo "Submitting EAGER SDSR training array"
...
bsub \
  -J "eager_sdsr_train[1-${NUM_DATASETS}]%${MAX_CONCURRENT_GPU}" \
  -q gpu \
  -gpu "num=1:mode=exclusive_process" \
  -n "${CPU_CORES:-1}" \
  -R "rusage[mem=${MEM_MB:-64000}]" \
  -W "${WALLTIME:-72:00}" \
  -o "${PROJECT_ROOT}/lsf_logs/eager_train_%J_%I.out" \
  -e "${PROJECT_ROOT}/lsf_logs/eager_train_%J_%I.err" \
