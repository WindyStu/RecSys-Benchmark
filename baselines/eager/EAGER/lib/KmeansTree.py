...
def select_cluster_index(index, choices, cluster_id):
    choices = choices.to(index.device)
    return index[choices == cluster_id]
...
class ConstructKmeansTree:
    ...
    def _kmeans(self,index):
        ...
        with torch.no_grad():
            ...
            choices, _ = kmeans_equal(self.data[:,selected_feature][index.to(self.data.device)], cluster_size=cluster_size,num_clusters=self.k,max_iters=self.max_iters)
        result_index=torch.full((self.k,cluster_size),-1,dtype=torch.int64,device=index.device)
        for c in range(self.k):
            result_index[c]=select_cluster_index(index, choices, c)
