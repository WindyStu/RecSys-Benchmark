...
class Train_instance:
    ...
    def read_all_instances_files(self,traing_instaces_path,seg_couts,item_num, his_maxtix=None,labels=None):
        his_cache_path = his_maxtix
        label_cache_path = labels
        if his_cache_path and label_cache_path and Path(his_cache_path).exists() and Path(label_cache_path).exists():
            his_maxtix = torch.load(his_cache_path)
        ...
        assert len(his_maxtix)==len(labels)
        if his_cache_path:
            Path(his_cache_path).parent.mkdir(parents=True, exist_ok=True)
            torch.save(his_maxtix, his_cache_path)
        if label_cache_path:
            Path(label_cache_path).parent.mkdir(parents=True, exist_ok=True)
