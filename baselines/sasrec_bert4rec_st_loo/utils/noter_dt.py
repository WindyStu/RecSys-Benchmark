...
class Noter(object):
    ...
    def __init__(self, args):
        ...
        self.f_log = join(args.path_log, f'{args.m}-dt-{args.data}-{time.strftime("%m-%d-%H-%M-", time.localtime())}-'
        ...
        # welcome
        self.log_msg('\n' + '-' * 20 + f' Experiment: {self.args.m}-dt ' + '-' * 20)
        self.log_settings()
    ...
    def log_final_result(self, res_a, res_b):
        ...
        self.log_settings()
        msg = (f'[ Info ] {self.args.m}-dt ({(time.time() - self.t_start) / 60:.1f} min)\n'
               f'      |                A                  |                B                  |\n'
