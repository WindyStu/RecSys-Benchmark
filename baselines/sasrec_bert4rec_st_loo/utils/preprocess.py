...
def reindex(df, df_a, df_b, list_u):
    ...
    df['item'] = [map_i[i][0] for i in df['item'].tolist()]

    # a
    df_a['user'] = [map_u[u] for u in df_a['user'].tolist()]
    df_a['item'] = [map_i_a[i][0] for i in df_a['item'].tolist()]

    # a
    df_b['user'] = [map_u[u] for u in df_b['user'].tolist()]
    df_b['item'] = [map_i_b[i][0] for i in df_b['item'].tolist()]

    print(f'\t   Done.')
    return df, df_a, df_b, map_u, map_i, map_i_a, map_i_b
...
def main():
    ...
    if args.data == 'afk':
        ...
        raise NotImplemented(f'Dataset {args.data} not found.')
    ...
    df, df_a, df_b, map_u, map_i, map_i_a, map_i_b = reindex(df, df_a, df_b, list_u)

    save(df, path_processed, f_processed, map_u, map_i)

    # save single-target datasets
    path_st_a = f'../data/{data_st_a}/'
    path_st_b = f'../data/{data_st_b}/'
    if not os.path.exists(path_st_a):
        os.makedirs(path_st_a)
    if not os.path.exists(path_st_b):
        os.makedirs(path_st_b)

    f_st_a = f'{data_st_a}_{args.len_max}_preprocessed.txt'
    f_st_b = f'{data_st_b}_{args.len_max}_preprocessed.txt'

    save(df_a, path_st_a, f_st_a, map_u, map_i_a)
    save(df_b, path_st_b, f_st_b, map_u, map_i_b)
