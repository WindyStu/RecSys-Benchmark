...
from dataloaders.dataloader_BERT4Rec_mt import get_dataloader
...
class Trainer(object):
    ...
    def train_batch(self, batch):
        ...
        loss = self.model.cal_rec_loss_st(h_mean, h_cov, gt, gt_neg, mask_seq)
        loss.backward()
        self.optimizer.step()
        return loss.item()
    ...
    def evaluate_batch(self, batch):
        ...
        h_mean, h_cov = self.model(seq, pos, mask_seq)
        return self.model.cal_rank_dt(h_mean[:, -1], h_cov[:, -1], gt, gt_mtc, mask_gt_a, mask_gt_b)
