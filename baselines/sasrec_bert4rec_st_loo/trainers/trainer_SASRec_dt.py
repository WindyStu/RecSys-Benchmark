...
from dataloaders.dataloader_SASRec_dt import get_dataloader
...
class Trainer(object):
    ...
    def train_batch(self, batch):
        seq, pos, mask_seq, gt, mask_seq_a, mask_seq_b = map(lambda x: x.to(self.device), batch)

        h_seq = self.model(seq, pos, mask_seq)
        loss = self.model.cal_rec_loss_dt(h_seq, gt, mask_seq_a, mask_seq_b)
