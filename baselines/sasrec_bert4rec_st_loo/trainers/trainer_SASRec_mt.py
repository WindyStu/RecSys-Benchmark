...
from dataloaders.dataloader_SASRec_mt import get_dataloader
...
class Trainer(object):
    ...
    def train_batch(self, batch):
        seq, pos, mask_seq, gt = map(lambda x: x.to(self.device), batch)

        h_seq = self.model(seq, pos, mask_seq)
        loss = self.model.cal_rec_loss_st(h_seq, gt, mask_seq)
