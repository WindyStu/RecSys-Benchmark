...
class CDSRDataset(Dataset):
    ...
    def process_train(self, seq_raw):
        ...
        return seq, pos, mask_seq, gt, mask_gt_a, mask_gt_b, seq_raw[:-2]
    ...
    def get_item_train(self, index):
        seq, pos, mask_seq, gt, mask_gt_a, mask_gt_b, seq_raw = self.data_tr[index]
        gt_neg = np.array([self.get_spe_neg(i, 1, seq_raw) for i in gt])

        return seq, pos, mask_seq, gt, gt_neg, mask_gt_a, mask_gt_b
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            data = self.get_item_train(index)
