...
for IDX in "${!DATASET_LIST[@]}"; do
  ...
  INDEX_FILE="../data/${DATASET}/${DATASET}.index.qwen-letter.json"
  ...
  if [[ ! -f "$INDEX_FILE" ]]; then
    ...
    echo "Run RQ-VAE/b_run_generate_indices_qwen_multi.sh \"${DATASET}\" first."
    continue
  fi
  ...
  "bash -lc 'export PYTHONUNBUFFERED=1; echo \"stage=LETTER-TIGER train_and_test dataset=${DATASET} time=${RUN_TS} index_file=${INDEX_FILE} output_dir=${OUTPUT_DIR}\"; \
