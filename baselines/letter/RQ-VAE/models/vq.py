...
class VectorQuantizer(nn.Module):
    ...
    def __init__(self, n_e, e_dim, mu = 0.25,
                 ...
                 sk_epsilon=0.01, sk_iters=100, use_diversity=True):
        super().__init__()
        self.n_e = n_e
        self.e_dim = e_dim
        self.beta = beta
    ...
    def constrained_km(self, data, n_clusters=10):
        ...
        if n_samples < n_clusters:
            # KMeans cannot create more centers than samples. Repeat samples only for
            # tiny/debug datasets; normal LETTER datasets have far more items.
            sample_idx = torch.randint(0, n_samples, (n_clusters,), device=data.device)
            centers = data[sample_idx].detach().cpu()
            return centers, [0 for _ in range(n_samples)]

        size_min = min(max(n_samples // (n_clusters * 2), 1), 50) # 50 for the very first time, 10 the latter
        size_max = max(size_min * 4, math.ceil(n_samples / n_clusters))

        logger = logging.getLogger()
        message = "constrained k-means started [samples=%d, clusters=%d, size_min=%d, size_max=%d]" % (
            n_samples,
            n_clusters,
            size_min,
            size_max,
        ...
        clf.fit(x)
        message = "constrained k-means finished [time=%.2fs]" % (time.time() - start_time)
        print(message, flush=True)
        logger.info(message)
    ...
    def forward(self,  x, label, idx, use_sk=True):
        ...
        # Diversity regularization is optional for ablation.
        if self.use_diversity and self.beta > 0:
            diversity_loss = self.diversity_loss_main_entry(x, x_q, indices, label)
        else:
            diversity_loss = torch.zeros((), device=x.device)
