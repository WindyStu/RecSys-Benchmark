...
for IDX in "${!DATASET_LIST[@]}"; do
  ...
  CKPT_ROOT="./checkpoint/${DATASET}_qwen_letter"
  OUTPUT_FILE="./data/${DATASET}/${DATASET}.index.qwen-letter.json"
  ...
  if [[ ! -d "$CKPT_ROOT" ]]; then
    ...
    echo "  ${DATASET}-32d-sasrec.pt is only CF embedding, not the trained RQ-VAE codebook."
    echo "  Run: bash RQ-VAE/b_run_train_tokenizer_qwen_multi.sh \"${DATASET}\""
    continue
  fi
  ...
  bsub -J "idx_${DATASET}" -N -q volta \
  -e "$LSF_ERR" \
  -o "$LSF_OUT" \
  -n 1 \
  -gpu "num=1:mode=exclusive_process" \
  "bash -lc 'export PYTHONUNBUFFERED=1; echo \"stage=generate_indices dataset=${DATASET} time=${RUN_TS} checkpoint_root=${CKPT_ROOT} output_file=${OUTPUT_FILE}\"; \
