...
from vqrec_index_validation import validate_ce_targets, validate_forward_indices
...
class VQRec(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len):
        code_embedding_rows = (
            self.reassigned_code_embedding.size(0)
            if self.index_assignment_flag and self.reassigned_code_embedding is not None
            else self.pq_code_embedding.num_embeddings
        )
        validate_forward_indices(
            item_seq=item_seq,
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == 'BPR':
            ...
            validate_ce_targets(pos_items, logits.size(1), self.POS_ITEM_ID)
