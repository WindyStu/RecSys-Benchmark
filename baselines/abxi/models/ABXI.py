...
class ABXI(nn.Module):
    ...
    def cal_rank_full(self,
                      h: torch.Tensor,
                      gt: torch.Tensor,
                      seen: torch.Tensor,
                      ) -> torch.Tensor:
        """Full-domain rank, masking items already seen in the input history."""
        gt = gt.squeeze(-1)
        ranks = torch.ones(gt.size(0), device=gt.device, dtype=torch.long)

        for is_a, start, end in (
            ((0 < gt) & (gt <= self.n_item_a), 1, self.n_item_a),
            (gt > self.n_item_a, self.n_item_a + 1, self.n_item),
        ):
            idx = is_a.nonzero(as_tuple=False).squeeze(-1)
            if idx.numel() == 0:
                continue

            items = torch.arange(start, end + 1, device=gt.device)
            logits = h[idx].matmul(self.ei(items).transpose(0, 1))

            seen_cur = seen[idx]
            seen_mask = (seen_cur >= start) & (seen_cur <= end)
            row_idx = torch.arange(idx.numel(), device=gt.device).unsqueeze(1).expand_as(seen_cur)
            col_idx = seen_cur - start
            valid_row = row_idx[seen_mask]
            valid_col = col_idx[seen_mask]
            if valid_row.numel() > 0:
                logits[valid_row, valid_col] = -torch.inf

            gt_col = gt[idx] - start
            gt_logits = h[idx].mul(self.ei(gt[idx])).sum(dim=1)
            logits[torch.arange(idx.numel(), device=gt.device), gt_col] = gt_logits
            ranks[idx] = logits.gt(gt_logits.unsqueeze(1)).sum(dim=1).add(1)
