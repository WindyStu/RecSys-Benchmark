...
class Noter(object):
    ...
    def log_train(self,
        ...
        msg = f'-epoch {i_epoch:>3} | tr  | los | {f"{loss_a:.4f}"[:6]} | {f"{loss_b:.4f}"[:6]} | {f"{loss_m:.4f}"[:6]} | {t_gap:>5.1f}s |'
        self.log_msg(msg)
    ...
    def log_valid(self,
        ...
        msg = f'           | val |     | {res_a[0]:.4f} | {res_a[1]:.4f} | {res_a[2]:.4f} | {res_a[3]:.4f} | {res_a[4]:.4f} | {res_b[0]:.4f} | {res_b[1]:.4f} | {res_b[2]:.4f} | {res_b[3]:.4f} | {res_b[4]:.4f} |'
        self.log_msg(msg)

    def log_test(self,
                 ranks: tuple[list[float], list[float]],
                 ) -> None:
        msg = f'           | te  |  *  | {ranks[0][0]:.4f} | {ranks[0][1]:.4f} | {ranks[0][2]:.4f} | {ranks[0][3]:.4f} | {ranks[0][4]:.4f} | {ranks[1][0]:.4f} | {ranks[1][1]:.4f} | {ranks[1][2]:.4f} | {ranks[1][3]:.4f} | {ranks[1][4]:.4f} |  *  |'
        self.log_msg(msg)
    ...
    def log_final(self,
        ...
        msg = (f'[Result] {self.args.name} ({(time.time() - self.t_start) / 60:.1f} min)\n'
               ...
               f'| {ranks[0][0]:.4f} | {ranks[0][1]:.4f} | {ranks[0][2]:.4f} | {ranks[0][3]:.4f} | {ranks[0][4]:.4f} | {ranks[1][0]:.4f} | {ranks[1][1]:.4f} | {ranks[1][2]:.4f} | {ranks[1][3]:.4f} | {ranks[1][4]:.4f} |\n')
        self.log_msg(msg)
