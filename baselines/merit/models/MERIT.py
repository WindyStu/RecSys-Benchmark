...
class MERIT(torch.nn.Module):
    ...
    def cal_rank(self,
        ...
        logits_mtc = logits[:, 1:]
        if gt_mtc_mask is not None:
            logits_mtc = logits_mtc.masked_fill(~gt_mtc_mask, float('-inf'))
        ranks = logits_mtc.gt(logits[:, 0:1]).sum(dim=1).add(1)
